...
void ConvertPasswordStringToPassword(void)
{
	...
	if ((gamestate.violence < 0) || (gamestate.violence > 3))
		gamestate.violence = 0;
}
...
boolean ParseBattleFile(void)
{
	...
	else
	{
		...
		for (index = battle_Normal; index < battle_NumBattleModes; index++)
		{
			...
			ReadInt("Speed", &temp);
			if ((temp >= bo_normal_speed) && (temp <= bo_fast_speed))
			{
				...
				(index != battle_Eluder))
			{
				...
				ReadInt("Ammo", &temp);
				if ((temp >= bo_one_shot) && (temp <= bo_infinite_shots))
				{
			...
			if ((temp >= bo_light_dark) && (temp <= bo_light_lightning))
			{
...
void SetBattleDefaultValues(void)
{
	...
	//  no config file, so select default values
	//
	for (index = battle_StandAloneGame; index < battle_NumBattleModes; index++)
	{
