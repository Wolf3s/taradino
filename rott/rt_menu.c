...
CP_itemtype CustomMenu[] = { { 2, "custom1\0", 'C', { .vv = CP_Keyboard } },
...
CP_itemtype NormalKeyMenu[] = { { 2, "\0", 'L', { .vv = DefineKey } },
								{ 1, "\0", 'R', { .vv = DefineKey } },
								{ 1, "\0", 'F', { .vv = DefineKey } },
								{ 1, "\0", 'B', { .vv = DefineKey } },
								{ 1, "\0", 'R', { .vv = DefineKey } },
...
CP_itemtype MouseBtnMenu[] = { { 2, "\0", 'B', { .vv = DefineMouseBtn } },
...
CP_itemtype JoyBtnMenu[] = { { 2, "\0", 'B', { .vv = DefineJoyBtn } },
...
CP_itemtype VMenu[] = { { 2, "msetv\0", 'S', { .vv = CP_ViolenceLevel } },
...
void SetUpControlPanel(void)
{
	...
	if (modemgame == true)
	{
		...
		if (consoleplayer != 0)
		{
			MainMenu[battlemode].routine =
				(menuptr){ .vv = BattleGamePlayerSetup };
		}
	}
}
...
int HandleMenu(CP_iteminfo *item_i, CP_itemtype *items, void (*routine)(int w))
{
	...
	x = item_i->x;
	if ((MenuNum == 4) || (MenuNum == 6) || (item_i->fontsize == mn_smallfont))
	{
		basey = item_i->y;
	...
	do
	{
		...
		if (MenuNum == 11)
		{
			if ((Keyboard[sc_PgUp]) && ((items + 1)->active != CP_Inactive))
			{
	...
	if (MenuNum && (MenuNum != 4) && (MenuNum != 6))
	{
		...
		if (color != -1)
		{
			...
			else
			{
				...
				DrawMenuBufIString(posx, posy - 1, item_i->names[which], color);
			}
		}
	}
...

{
	...
	which = HandleMenu(&TufItems, &TufMenu[ToughMenuNum][0], DrawNewGameDiff);
...

{
	...
	for (i = 0; i < 6; i++)
	{
		...
		strcpy(&MouseBtnNames[i][MOUSEBTNINDEX], ControlNames[num]);
	}
...
#define CALY 22
	...
	unsigned short xmax, ymax, xmin, ymin, jb;
	int checkbits;
	int status;
	boolean done;
...
void CP_BattleMenu(void)
{
	...
	if ((gamestate.battlemode == battle_Tag) || (numplayers < 2))
	{
...
void CP_BattleModes(void)
{
	...
	do
	{
		which = HandleMenu(&ModeItems, &ModeMenu[0], DrawBattleModeDescription);
	} while (which >= 0);
	...
	else
	{
		...
		if (gamestate.Product != ROTT_SITELICENSE)
		{
			if (networkgame == true)
			{
				...
				{
					PrintBattleOption(true, 68, 77 + i * 8, sitemessage[i]);
				}
			}
		}
	}
...
//
//****************************************************************************
void DrawEnterCodeNameMenu(void)
...
//
//******************************************************************************
char *GetNextWord(char *dest, char *source, int length)
